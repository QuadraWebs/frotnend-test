                  ...
                  <div v-for="income in filteredIncomes" :key="income.id" 
                    ...
                    <div class="flex justify-between items-center mb-2">
                      <span class="px-2 py-1 text-xs font-medium rounded-full"
                        :class="getCategoryClass(income.category)">
...
const averageIncome = computed(() => {
  return incomes.value.length ? totalIncome.value / incomes.value.length : 0;
});
...
const categoryDistribution = computed(() => {
  ...
  ['salary', 'freelance', 'investment', 'rental', 'business', 'other'].forEach(category => {
    categoryMap.set(category, { name: category, amount: 0, count: 0 });
  });
  
  // Sum up amounts by category
  incomes.value.forEach(income => {
    const category = categoryMap.get(income.category);
    if (category) {
  ...
  const result = Array.from(categoryMap.values())
    ...
    .map(category => ({
      ...category,
      percentage: (category.amount / totalIncome.value) * 100
    }))
...
const fetchIncomes = async () => {
  ...
  try {
    ...
    // For demo purposes, if the API isn't available, use sample data
    if (process.env.NODE_ENV === 'development' && !response.data.data) {
      incomes.value = getSampleIncomes();
    }
  } catch (err: any) {
...
const confirmDelete = async () => {
  ...
  try {
    // This would be replaced with your actual API endpoint
    await axios.delete(`http://localhost:8000/api/incomes/${itemToDeleteId.value}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
...
const saveIncome = async () => {
  ...
  try {
    if (editingIncome.value && currentIncome.value.id) {
      ...
      const response = await axios.put(`http://localhost:8000/api/incomes/${currentIncome.value.id}`, currentIncome.value, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      if (index !== -1) {
        incomes.value[index] = response.data.data || currentIncome.value;
      }
    } else {
      // Add new income
      const response = await axios.post('http://localhost:8000/api/incomes', currentIncome.value, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
    ...
    if (process.env.NODE_ENV === 'development') {
      if (editingIncome.value && currentIncome.value.id) {
        // Update existing income in the list
        const index = incomes.value.findIndex(income => income.id === currentIncome.value.id);
        if (index !== -1) {
          incomes.value[index] = { ...currentIncome.value };
        }
      } else {
